...
class WpakSimulator{
	...
	public static function add_settings_panels(){
		add_submenu_page(WpakApps::menu_item,__('Simulator'), __('Simulator'), 'manage_options', self::menu_item, array(__CLASS__,'settings_panel'));
	}
	...
	public static function add_action_link($actions){
		...
		if( $post->post_type == 'wpak_apps' ) {
			if( array_key_exists('trash',$actions) ){
				...
				unset($actions['trash']);
				$actions['wpak-simulate-app'] = '<a href="'. self::get_simulator_url($post->ID) .'">'. __('View in simulator') .'</a>';
				$actions['wpak-view-app-in-browser'] = '<a href="'. WpakBuild::get_appli_index_url($post->ID) .'">'. __('View in browser') .'</a>';
		...
		<div class="wrap">
				...
				<?php if( WpakApps::app_exists($app_id) ): ?>
					...
					<div id="debug-infos">
						...
						<h3><a href="<?php echo $appli_dir_url .'/config.js?wpak_app_id='. $app_id ?>"><?php _e('View config.js') ?></a></h3>
